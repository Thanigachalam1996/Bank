...
const EditProfile = props => {
  ...
  const [value, setValue] = useState({
    ...
    useEffect(() => {
       ...
       let result=axios.get('https://localhost:9090/api/profile/${id}')
            .then( state => {
                const fields=['username', 'email', 'city', 'password', 'guardianType', 'guardianName',
              'address', 'citizenship', 'state', 'country', 'email', 'gender', 'maritalStatus',
            'contactNum', 'dob', 'registrationDate', 'accountType', 'branchName', 'citizenStatus',
          'depositAmount','proofType','documentNum','referenceAccName', 'referenceAccNum', 'refAccountAdd']
              fields.forEach(field => setValue(field, state[field])); 
              console.log(state);
            })
      ...
      <Form className="register-form" noValidate validated={validated}  onSubmit={handleSubmit}>
        ...
        <Form.Group as={Col} controlId="username">
          ...
          <Form.Control required
            type="text"
            placeholder="Enter username" pattern="^[a-zA-Z]+(\s[a-zA-Z]+)?$"
            name="username"
